...
class db {
	 ...
	 * @return mixed
	 */
	public static function create_fulltext_item($title, $content, $keywords, $url, $description)
	{
		...
		@self::getWriteFT_PrepareSql(
			$sql = "INSERT INTO search_data
                    ( title, content, keywords, url, description )
                    VALUES
                    (
                        :title ,
                        :content,
		...
		return $con = self::getInstance('fulltext-write')->getConnection()->lastInsertId();
	}
}
